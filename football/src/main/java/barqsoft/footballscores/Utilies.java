...
public class Utilies
{
    ...
    public static String getMatchDay(final Context context, int match_day,int league_num)
    {
        if(league_num == CHAMPIONS_LEAGUE)
        {
            if (match_day <= 6)
            {
                String prefix = context.getString(R.string.group_stage_text) + ", " + context.getString(R.string.matchday_text);
                return generateMatchdayString(context, prefix, 6);
            }
        ...
        {
            return generateMatchdayString(context, context.getString(R.string.matchday_text), match_day);
        }
    }

    private static String generateMatchdayString(final Context context, final String prefix, final int matchDay) {
        return context.getString(R.string.match_day_format, prefix, matchDay);
    }
